...
if(!code.includes('SIMANTAB_ACTIVITY_ATTENDANCE_RECAP_V1'))throw new Error('Modul attendance recap tidak valid.');
html=html.replace(/<script type="module" src="\.\/activity-attendance-recap\.js\?v=\d+"><\/script>\s*/g,'');
html=html.slice(0,bodyClose)+`<script type="module" src="./activity-attendance-recap.js?v=1"></script>\n`+html.slice(bodyClose);
await fs.writeFile(`.vercel/output/static/${moduleName}`,code);
await fs.writeFile(outputPath,html);
console.log(JSON.stringify({attendanceRecap:true,version:1,printPdf:true,excelExport:true,signatureIncluded:true}));
