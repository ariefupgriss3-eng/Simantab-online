...
html=html.slice(0,bodyClose)+`<script type="module" src="./activity-attendance-success-ux.js?v=1"></script>\n`+html.slice(bodyClose);
await fs.writeFile(`.vercel/output/static/${moduleName}`,code);
await fs.writeFile(outputPath,html);
console.log(JSON.stringify({attendanceSuccessUx:true,nipOnly:true,nipRequired:true,successSummary:['name','nip','unit','position','time'],mobileOptimized:true,productionUntouched:true}));
