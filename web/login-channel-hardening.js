/* SIMANTAB_LOGIN_CHANNEL_HARDENING_V1 */
(async()=>{
...
function updateUi(){
 ...
 input.placeholder=ch==='GTK'?'username KS/GTK atau email':'username Dinas atau email';
 if(ch==='GTK'&&hint)hint.innerHTML='<b>Login GTK:</b> gunakan <b>username atau email</b> akun GTK/Kepala Sekolah. Akun Dinas tidak dapat masuk melalui kanal ini.';
}
function effectiveChannel(prof){
 ...
 return GTK_FALLBACK.has(String(prof?.role||'').toUpperCase())?'GTK':'DINAS';
}
...
async function hardenedLogin(){
 ...
 try{
  ...
  const {data:prof,error}=await sb.from('profiles').select('role,is_active,approval_status,account_channel,full_name').eq('id',uid).maybeSingle();if(error)throw error;
  ...
  if(actual!==wanted){await sb.auth.signOut();throw new Error(actual==='GTK'?'Akun ini adalah akun GTK/Kepala Sekolah. Gunakan Login GTK.':'Akun ini adalah akun Dinas. Gunakan Login Dinas.');}
  location.reload();
...
if(oldChannel)window.setChannel=c=>{oldChannel(c);setTimeout(updateUi,0)};
