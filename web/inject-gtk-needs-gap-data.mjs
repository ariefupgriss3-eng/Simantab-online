...
if(!/SIMANTAB_GTK_NEEDS_GAP_DATA_V1/.test(code))throw new Error('GTK needs Gap Data v1 tidak valid.');
...
const tag='<script type="module" src="./gtk-needs-gap-data.js?v=1"></script>\n';
html=html.slice(0,close)+tag+html.slice(close);
if(html.split('./gtk-needs-gap-data.js?v=1').length-1!==1)throw new Error('Referensi Gap Data harus tepat satu kali.');
await fs.writeFile(outputPath,html);
console.log(JSON.stringify({gtkNeedsGapData:true,version:1,position:'right-of-gap-riil'}));
