...
if(!/SIMANTAB_GTK_NEEDS_GAP_DATA_V3/.test(code))throw new Error('GTK needs Gap Data v3 tidak valid.');
...
const tag='<script type="module" src="./gtk-needs-gap-data.js?v=3"></script>\n';
html=html.slice(0,close)+tag+html.slice(close);
if(html.split('./gtk-needs-gap-data.js?v=3').length-1!==1)throw new Error('Referensi Gap Data v3 harus tepat satu kali.');
await fs.writeFile(outputPath,html);
console.log(JSON.stringify({gtkNeedsGapData:true,version:3,position:'right-of-gap-riil',dinasSource:'VERIFIED-only'}));
