...
const FILE_LIMIT = 512000; // 500 KiB displayed as 500 KB
...
// Global upload policy: every SIMANTAB document is limited to 500 KB.
html=html
  .replaceAll('maksimal 1 MB per berkas','maksimal 500 KB per berkas')
  .replaceAll('maksimal 1 MB per file','maksimal 500 KB per file')
  .replaceAll('batas 1 MB','batas 500 KB')
  .replaceAll('f.size<=1048576',`f.size<=${FILE_LIMIT}`)
  .replaceAll('f.size>1048576',`f.size>${FILE_LIMIT}`);

enhancement=enhancement
  .replaceAll('1048576',String(FILE_LIMIT))
  .replaceAll('1 MB','500 KB');
...
const externalKpPos = html.lastIndexOf('kp-enhancement.js?v=4');
