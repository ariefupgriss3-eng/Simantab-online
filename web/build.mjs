...
async function getText(url, fallback='') {
  try {
    const r = await fetch(url, {headers:{'user-agent':'SIMANTAB-Vercel-Build/1.2'}, cache:'no-store'});
...
function stripBrokenInlineKp(source){
  ...
  for(let guard=0;guard<4 && html.includes(marker);guard++){
    ...
    const start=html.lastIndexOf('<script type="module">', markerPos);
    const end=html.indexOf('</script>', markerPos);
...
function checkModuleSyntax(name, code){
  ...
  return fs.writeFile(path,code).then(()=>{
    const r=spawnSync(process.execPath,['--check',path],{encoding:'utf8'});
    if(r.status!==0) throw new Error(`Syntax check ${name} gagal: ${(r.stderr||r.stdout||'').trim()}`);
  });
}
...
// Recover the production HTML if the previous KP patch was accidentally inserted
// inside the print-document template string.
const recovered=stripBrokenInlineKp(html);
html=recovered.html;

// Idempotency: remove a previously correct external KP tag before adding the current one.
html=html.replace(externalTagRe,'');
...
// Insert at the LAST body close. The text </body> also occurs inside a print-template.
const bodyClose = html.lastIndexOf('</body>');
if(bodyClose < 0) throw new Error('Tag penutup body tidak ditemukan.');
const kpTag = '<script type="module" src="./kp-enhancement.js?v=3"></script>\n';
html = html.slice(0, bodyClose) + kpTag + html.slice(bodyClose);

// Validate placement and syntax before Vercel is allowed to publish the bundle.
const mainOpen='<script type="module">';
const mainScriptStart = html.indexOf(mainOpen);
const mainScriptEnd = html.indexOf('</script>', mainScriptStart+mainOpen.length);
const externalKpPos = html.lastIndexOf('kp-enhancement.js?v=3');
...
await checkModuleSyntax('simantab-kp',enhancement);

if(!html.includes('window.__simantabSb=sb;') || !html.includes('window.__simantabProfile=profile;')) throw new Error('Global bridge SIMANTAB untuk modul KP tidak lengkap.');
...
// Network-first and purge all prior caches to remove the broken bundle from devices.
const sw = `const CACHE_VERSION='simantab-v9';self.addEventListener('install',()=>self.skipWaiting());self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.map(k=>caches.delete(k)))).then(()=>self.clients.claim())));self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;e.respondWith(fetch(e.request,{cache:'no-store'}).catch(()=>caches.match(e.request)))})`;
...
console.log(JSON.stringify({
  ok:true,
  recoveredInlineKp:recovered.removed,
  mainSyntax:true,
  kpSyntax:true,
