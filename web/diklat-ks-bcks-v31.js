...
const draftLast=(a,b)=>Number((a.workflow_state||a.admin_status)==='DRAFT')-Number((b.workflow_state||b.admin_status)==='DRAFT');
const leadershipOrder=(a,b)=>{const rank=s=>s==='MENUNGGU_PERSETUJUAN_KABID'?0:s==='SELESAI'?2:s==='DRAFT'?3:1;return rank(a.workflow_state)-rank(b.workflow_state)};
...
function ensureKsbAssignStyle(){
 ...
 .ksb-kabid-celebration{position:relative;overflow:hidden;margin:0 0 14px;padding:16px 18px;border:1px solid #f6c453;border-radius:16px;background:linear-gradient(135deg,#fff9db,#fff4bd 48%,#fffdf1);box-shadow:0 8px 24px #e3a00822;color:#714b00}
 .ksb-kabid-celebration:before,.ksb-kabid-celebration:after{content:'✦  ★  ✨  ★  ✦';position:absolute;left:-40%;width:140%;font-size:17px;letter-spacing:18px;color:#e8a600;opacity:.75;white-space:nowrap;pointer-events:none;animation:ksbStars 8s linear infinite}
 .ksb-kabid-celebration:before{top:7px}.ksb-kabid-celebration:after{bottom:5px;animation-direction:reverse;animation-duration:10s}
 .ksb-kabid-title{font-size:11px;font-weight:950;letter-spacing:.08em;text-transform:uppercase;color:#946200;margin-bottom:8px}
 .ksb-kabid-message{position:relative;z-index:1;font-weight:900;font-size:15px;line-height:1.55;text-align:center;animation:ksbMessageFloat 2.8s ease-in-out infinite}
 .ksb-kabid-meta{position:relative;z-index:1;margin-top:8px;text-align:center;font-size:10px;color:#876516}
 @keyframes ksbStars{from{transform:translateX(0)}to{transform:translateX(38%)}}
 @keyframes ksbMessageFloat{0%,100%{transform:translateY(0);text-shadow:0 0 0 #f6c45300}50%{transform:translateY(-3px);text-shadow:0 3px 12px #e6a60055}}
 @media(prefers-reduced-motion:reduce){.ksb-kabid-celebration:before,.ksb-kabid-celebration:after,.ksb-kabid-message{animation:none}}
 @media(max-width:760px){.ksb-staff-grid{grid-template-columns:1fr}.ksb-kabid-message{font-size:13px}}
...
function kabidCelebrationHtml(d){
 ...
 if(!d?.kabid_approved_at||!note)return'';
 return `<div class="ksb-kabid-celebration" role="status" aria-label="Ucapan persetujuan Kabid"><div class="ksb-kabid-title">⭐ Ucapan Kabid Ketenagaan ⭐</div><div class="ksb-kabid-message">${esc(note)}</div><div class="ksb-kabid-meta">Disetujui Kabid • ${esc(fmtDateTime(d.kabid_approved_at))}</div></div>`;
}
