...
function leaderSurplusRows(d){const schoolBy=new Map((d.schools||[]).filter(s=>String(s.school_status||'').toUpperCase()==='NEGERI').map(s=>[s.npsn,s])),grouped=new Map();for(const x of d.needs||[]){const school=schoolBy.get(x.school_npsn);if(!school||!visibleSurplusNeed(x))continue;const category=surplusCategory(x);if(!category)continue;const position=surplusPosition(x),key=[x.school_npsn,category,position].join('|'),g=grouped.get(key)||{school,category,position,abk:0,asn:0,non:0};g.abk+=num(x.abk);g.asn+=num(x.asn_total);g.non+=num(x.non_asn_total);grouped.set(key,g)}const out=[];for(const g of grouped.values()){const surplusAsn=Math.max(0,g.asn-g.abk),surplusTotal=Math.max(0,g.asn+g.non-g.abk);if(surplusTotal>0)out.push({...g,surplusAsn,surplusTotal})}return out.sort((a,b)=>b.surplusTotal-a.surplusTotal||b.surplusAsn-a.surplusAsn||String(a.school.school_name||'').localeCompare(String(b.school.school_name||''),'id'))}
function surplusStats(rows,category){const hit=rows.filter(x=>x.category===category);return{schools:new Set(hit.map(x=>x.school.npsn)).size,positions:hit.length,total:hit.reduce((n,x)=>n+x.surplusTotal,0),asn:hit.reduce((n,x)=>n+x.surplusAsn,0)}}
...
${card('📈','Sekolah Negeri Kelebihan Guru',`${fmt(sg.schools)} sekolah`,`${fmt(sg.positions)} jabatan • Kelebihan ASN ${fmt(sg.asn)} • Total ${fmt(sg.total)}`,"__leaderOpenSurplusDetails('Guru')")}
${card('🗂️','Sekolah Negeri Kelebihan TAS',`${fmt(st.schools)} sekolah`,`${fmt(st.positions)} jabatan • Kelebihan ASN ${fmt(st.asn)} • Total ${fmt(st.total)}`,"__leaderOpenSurplusDetails('TAS')")}
${card('🛡️','Sekolah Negeri Kelebihan Penjaga',`${fmt(sp.schools)} sekolah`,`${fmt(sp.positions)} jabatan • Kelebihan ASN ${fmt(sp.asn)} • Total ${fmt(sp.total)}`,"__leaderOpenSurplusDetails('Penjaga')")}
...
window.__leaderOpenSurplusDetails=async function(category){try{
 if(!isLeader())return;const allowed=['Guru','TAS','Penjaga'];if(!allowed.includes(category))return;
 ...
 const total=rows.reduce((n,x)=>n+x.surplusTotal,0),asn=rows.reduce((n,x)=>n+x.surplusAsn,0),schools=new Set(rows.map(x=>x.school.npsn)).size;
 m.innerHTML=`<div class="card" style="width:min(1180px,100%);max-height:92vh;overflow:auto"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div><div class="label">INFOGRAFIS GTK • SEKOLAH NEGERI • DATA TERVERIFIKASI</div><h3 style="margin:3px 0">Kelebihan ${esc(category)}</h3><div class="small">${fmt(schools)} sekolah • kelebihan ASN ${fmt(asn)} • kelebihan total ${fmt(total)}</div></div><button class="btn soft" onclick="document.getElementById('leaderSurplusDetailModal')?.remove()">✕</button></div><div class="info" style="margin:12px 0"><b>Rumus:</b> Kelebihan ASN = ASN − ABK; Kelebihan Total = ASN + Non-ASN − ABK. Hanya nilai positif per jabatan yang ditampilkan.</div><div class="tablewrap"><table><thead><tr><th>No</th><th>Sekolah</th><th>NPSN</th><th>Jenjang</th><th>Kecamatan</th><th>Jabatan</th><th>ABK</th><th>ASN</th><th>Non-ASN</th><th>Kelebihan ASN</th><th>Kelebihan Total</th></tr></thead><tbody>${rows.length?rows.map((x,i)=>`<tr><td>${i+1}</td><td><b>${esc(x.school.school_name)}</b></td><td>${esc(x.school.npsn)}</td><td>${esc(x.school.jenjang||x.school.bentuk_pendidikan||'-')}</td><td>${esc(x.school.kecamatan||'-')}</td><td>${esc(x.position)}</td><td>${fmt(x.abk)}</td><td>${fmt(x.asn)}</td><td>${fmt(x.non)}</td><td><span class="kpill">${fmt(x.surplusAsn)}</span></td><td><span class="kpill">${fmt(x.surplusTotal)}</span></td></tr>`).join(''):`<tr><td colspan="11"><div class="ks">Tidak ada sekolah negeri yang kelebihan ${esc(category)} pada data terverifikasi.</div></td></tr>`}</tbody></table></div></div>`;
 document.body.appendChild(m);
}catch(e){alert(e.message||String(e))}};
