...
function renderLeaderSummary(s,live=false){
 ...
 b.innerHTML=`<div class="kdg">
  ...
  ${eduUnitPanel()}
  ${card('🏫','Total Satuan Pendidikan',fmt(EDU_BATANG.total),`Formal ${fmt(EDU_BATANG.formal.total)} • Nonformal ${fmt(EDU_BATANG.nonformal.total)} • Negeri ${fmt(EDU_BATANG.status.negeri)} • Swasta ${fmt(EDU_BATANG.status.swasta)}`)}
...
async function dash(force=false){
 ...
 try{
  ...
  b.innerHTML=`<div class="kdg"><div class="kh k12"><h2>Command Center Ketenagaan</h2><p>${esc(rt)} • ${esc(scopeText)} • kondisi sekolah, GTK, layanan, dan isu prioritas.</p></div>
   ${requesterSearchPanel()}
  ${eduUnitPanel()}
   ${card('🏫','Total Satuan Pendidikan',fmt(EDU_BATANG.total),`Formal ${fmt(EDU_BATANG.formal.total)} • Nonformal ${fmt(EDU_BATANG.nonformal.total)} • N ${fmt(EDU_BATANG.status.negeri)} • S ${fmt(EDU_BATANG.status.swasta)}`)}
