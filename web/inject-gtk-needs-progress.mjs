...
const summaryFn=`function schoolSummaryRows(schools,needs,workflow,mode){
 ...
 return orderedSchools.map(s=>{
  ...
  return \`<tr><td><b>\${esc(s.school_name)}</b><div class="sim-needs-note">\${esc(s.npsn)} • \${esc(s.jenjang||s.bentuk_pendidikan||'-')} • \${esc(s.kecamatan||'-')} • NEGERI</div>\${ksInvalid?'<div class="sim-needs-warn" style="margin-top:6px"><b>⚠ ABK Kepala Sekolah harus 1.</b><br>Distribusikan ABK ke masing-masing jabatan/mapel sebelum verifikasi.</div>':''}\${detail}</td><td><span class="sim-needs-status \${STATUS_CLASS[st]||'not'}">\${esc(st==='SUBMITTED'?'Perlu Verifikasi':(st==='APPROVED'?'Diverifikasi':(STATUS_LABEL[st]||st)))}</span>\${wf?.note?\`<div class="sim-needs-note">\${esc(wf.note)}</div>\`:''}</td><td><b>\${num(s.rombel)}</b></td><td>\${a.abk}</td><td>\${a.asn}</td><td>\${a.non}</td><td>\${gapButton(a.gap,gapKey,'Gap Riil')}<div class="sim-needs-note">per jabatan</div></td><td>\${gapButton(a.gapData,gapDataKey,'Gap Data')}<div class="sim-needs-note">per jabatan</div></td><td>\${ksInvalid?'<span class="sim-needs-status revision">Perlu Koreksi ABK</span>':action}</td></tr>\`;
 }).join('')
}
...
const scopedFn=`async function renderScoped(box,mode){
 ...
 if(mode==='PENGAWAS'){
  ...
  const {data,error}=await client.from('school_master').select('npsn,school_name,school_status,jenjang,bentuk_pendidikan,kecamatan,rombel,is_active').eq('is_active',true).eq('school_status','NEGERI').order('school_name');if(error)throw error;schools=data||[];
