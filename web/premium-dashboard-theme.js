...
.sim-premium-bottom{display:grid;grid-template-columns:1.35fr 1fr;gap:12px;margin-top:12px}.sim-premium-panel{background:#fff;border:1px solid #e0eaf3;border-radius:15px;padding:14px;box-shadow:0 6px 18px rgba(10,53,104,.055)}.sim-premium-panel h4{margin:0 0 10px;color:var(--navy)}.sim-premium-row{padding:9px 0;border-bottom:1px solid #edf2f6;font-size:11px;color:#516a80}.sim-premium-row:last-child{border-bottom:0}.sim-premium-date{display:inline-grid;place-items:center;width:42px;height:42px;border-radius:11px;background:#edf5fc;color:var(--navy);font-weight:900;margin-right:9px;vertical-align:middle}.sim-premium-motto{text-align:center;margin:20px 0 4px;font-size:10px;font-weight:900;letter-spacing:.08em;color:#54718a}.sim-premium-motto b{color:var(--orange)}
...
function enhance(){
 ...
 if(items.length){
  ...
  sec.innerHTML=`<div class="sim-premium-section-title"><h3>Menu Layanan Utama</h3><span>Pilih menu untuk membuka layanan</span></div><div class="sim-premium-tiles">${items.map(d=>`<button class="sim-premium-tile" data-premium-tab="${d[0]}"><div class="ico2">${d[1]}</div><b>${d[2]}</b><small>${d[3]}</small><span class="arrow">→</span></button>`).join('')}</div><div class="sim-premium-bottom"><div class="sim-premium-panel"><h4>📣 Pengumuman</h4><div class="sim-premium-row"><b>SIMANTAB aktif</b><br>Gunakan modul sesuai kewenangan dan alur verifikasi yang berlaku.</div><div class="sim-premium-row"><b>Data & dokumen</b><br>Pastikan data dan berkas yang diunggah benar sebelum dikirim.</div></div><div class="sim-premium-panel"><h4>🗓️ Agenda</h4><div class="sim-premium-row"><span class="sim-premium-date">NOW</span><b>Kegiatan Bidang</b><br><span class="small">Lihat agenda terbaru pada menu Kegiatan Bidang.</span></div></div></div><div class="sim-premium-motto">SDM PENDIDIKAN UNGGUL, <b>BATANG SEMAKIN MAJU</b></div>`;
  body.appendChild(sec);sec.querySelectorAll('[data-premium-tab]').forEach(b=>b.addEventListener('click',()=>window.showTab?.(b.dataset.premiumTab)));
 }
}
decorateStatic();
(async()=>{
