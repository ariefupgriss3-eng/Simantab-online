...
// Revisi berkas Seleksi Administrasi: peserta hanya mengunggah 6 dokumen.
...
if(!code.includes(oldRequirements) && !code.includes(newRequirements))throw new Error('Daftar berkas Seleksi Administrasi tidak ditemukan.');
code=code.replace(oldRequirements,newRequirements);

const bodyClose=html.lastIndexOf('</body>');
if(bodyClose<0)throw new Error('Tag </body> tidak ditemukan.');
const tag=`<script type="module" src="./${moduleName}?v=6"></script>\n`;
...
console.log(JSON.stringify({diklatKsBcks:true,version:2,levels:4,level4:'PENCATATAN_SERTIFIKAT',certificateFlow:'PESERTA_ISI_ADMIN_KSPS_APPROVE',adminRequiredFiles:['SKP_1','SKP_2','SK_PENGALAMAN_MANAJERIAL','SK_HUDIS','SKCK','PAKTA_INTEGRITAS'],adminRequiredFileCount:6,fileLimitBytes:512000,adminKsps:'kasim',productionUntouched:true}));