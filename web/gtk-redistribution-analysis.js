...
function studyClusterRows(study){
 return study.clusters.map((x,i)=>{
  ...
  return'<tr><td>'+(i+1)+'</td><td><b>'+esc(x.label)+'</b><div class="gar-small">'+esc(x.access)+'</div></td><td>'+esc(x.district)+'</td><td>'+baseline+'</td><td>±'+String(x.studyDistanceKm).replace('.',',')+' km<div class="gar-small">indikatif kajian</div></td><td>'+esc(live)+'</td><td>'+gtk+'</td><td>'+abk+'</td><td>'+gap+'</td><td><span class="gar-study-nd">ND</span><div class="gar-small">Butuh murid per kelas I–VI, kapasitas ruang, dan linearitas.</div></td><td><span class="gar-pill orange">Studi kelayakan</span><div class="gar-small">Belum keputusan merger.</div></td></tr>';
 }).join('');
}
function studySection(m){
 const s=m.study;if(!s?.visible)return'';
 s.wf=m.wf;
 return'<div id="garStudyPanel" class="gar-card gar-wide"><div class="gar-study-title"><div><div class="gar-label">INTEGRASI KAJIAN PENATAAN SD</div><h3 style="margin:5px 0 4px;color:#0f3f76">Penataan SD ↔ Redistribusi GTK</h3><div class="gar-small">Kajian V2 dipakai sebagai <b>decision-support</b>. Empat klaster Tahap I tetap objek studi kelayakan, bukan keputusan penggabungan. SIMANTAB <b>tidak menambah surplus guru dari skenario merger</b> sebelum legal gate terpenuhi dan ABK sesudah skenario dapat dihitung.</div></div><span class="gar-study-flag gar-study-ok">ACUAN TUNGGAL • '+esc(SD_STUDY.version)+' • '+esc(SD_STUDY.date)+'</span></div>'+
 '<div class="gar-study-kpis"><div class="gar-study-kpi"><span>SD Negeri — acuan tunggal</span><b>'+fmt(SD_STUDY.baselineSchools)+'</b></div><div class="gar-study-kpi"><span>Total peserta didik</span><b>'+fmt(SD_STUDY.baselineStudents)+'</b></div><div class="gar-study-kpi"><span>SD ≤90 peserta didik</span><b>'+fmt(SD_STUDY.low90)+'</b></div><div class="gar-study-kpi"><span>SD ≤120 peserta didik</span><b>'+fmt(44+93+87)+'</b></div><div class="gar-study-kpi"><span>Klaster Tahap I kajian</span><b>'+fmt(SD_STUDY.clusters.length)+'</b></div></div>'+
 '<div class="gar-note"><b>Angka acuan dikunci:</b> untuk Kajian Penataan SD digunakan satu hitungan, yaitu baseline Data Pendidikan/Kemendikdasmen cut-off '+esc(SD_STUDY.studentCutoff)+': <b>445 SD Negeri, 58.670 peserta didik, dan 137 SD dengan ≤90 peserta didik</b>. Data operasional SIMANTAB tetap dipakai untuk ABK dan redistribusi GTK, tetapi tidak ditampilkan sebagai pembanding baseline agar tidak menimbulkan dua angka acuan.</div>'+
 ...
 '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px"><div><div class="gar-label">LEGAL GATE PASAL 24 — PERBUP BATANG NOMOR 1 TAHUN 2020</div><div class="gar-small" style="margin:4px 0 7px"><b>Dasar hukum:</b> '+esc(SD_STUDY.legalBasis)+'</div><div class="gar-table-wrap" style="max-height:360px"><table class="gar-table"><thead><tr><th>Kode</th><th>Syarat</th><th>Status Kajian V2</th></tr></thead><tbody>'+studyLegalRows()+'</tbody></table></div></div><div><div class="gar-label">SCREENING KUANTITATIF — ACUAN TUNGGAL</div><div class="gar-table-wrap" style="max-height:360px"><table class="gar-table"><thead><tr><th>Kategori</th><th>Jumlah SD</th><th>Persentase</th></tr></thead><tbody><tr><td>≤60</td><td>'+fmt(44)+'</td><td>9,9%</td></tr><tr><td>61–90</td><td>'+fmt(93)+'</td><td>20,9%</td></tr><tr><td>91–120</td><td>'+fmt(87)+'</td><td>19,6%</td></tr><tr><td>&gt;120</td><td>'+fmt(221)+'</td><td>49,7%</td></tr></tbody></table></div><div class="gar-small" style="margin-top:7px">Kategori adalah alat screening, bukan syarat hukum penggabungan.</div></div></div>'+
 '<div style="margin-top:12px"><div class="gar-label">EMPAT KLASTER TAHAP I — TANPA PERINGKAT</div><div class="gar-table-wrap"><table class="gar-table"><thead><tr><th>No</th><th>Klaster</th><th>Kec.</th><th>PD Acuan</th><th>Jarak Kajian</th><th>Jarak SIMANTAB</th><th>Status GTK</th><th>ABK/ASN berjalan</th><th>Gap/Surplus berjalan</th><th>ABK sesudah skenario</th><th>Status</th></tr></thead><tbody>'+studyClusterRows(s)+'</tbody></table></div></div>'+
 '<div class="gar-study-source" style="margin-top:10px">Acuan tunggal: '+esc(SD_STUDY.title)+' — '+esc(SD_STUDY.version)+', '+esc(SD_STUDY.date)+'; dasar hukum '+esc(SD_STUDY.legalBasis)+'; cut-off peserta didik '+esc(SD_STUDY.studentCutoff)+'. Jarak kajian bersifat indikatif dan tidak sama dengan rute murid.</div></div>';
}
...
function downloadCsv(m){
 ...
 if(m.study?.visible){
  lines.push([],['KAJIAN PENATAAN SD V2 — ACUAN TUNGGAL'],['Dokumen',SD_STUDY.title],['Tanggal Kajian',SD_STUDY.date],['Cut-off peserta didik',SD_STUDY.studentCutoff],['SD Negeri',SD_STUDY.baselineSchools],['Total peserta didik',SD_STUDY.baselineStudents],['SD ≤90 peserta didik',SD_STUDY.low90],['SD ≤120 peserta didik',44+93+87],['R-SDM baseline ABK',SD_STUDY.rsdm.abk],['R-SDM baseline ASN',SD_STUDY.rsdm.asn],['Selisih dashboard R-SDM',SD_STUDY.rsdm.displayGap],['Cut-off R-SDM',SD_STUDY.rsdm.cutoff],[],['KLASTER TAHAP I — STUDI KELAYAKAN, BUKAN KEPUTUSAN MERGER'],['Klaster','Kecamatan','PD Acuan','Jarak Kajian km','Jarak SIMANTAB km','Status GTK','ABK Berjalan','ASN Berjalan','Gap Berjalan','Surplus Berjalan','ABK Sesudah Skenario','Status']);
  m.study.clusters.forEach(x=>lines.push([x.label,x.district,x.schools.reduce((n,s)=>n+s.students,0),x.studyDistanceKm,x.liveDistance??'Belum ada',x.bothVerified?'2/2 terverifikasi':'Belum resmi',x.metrics?.abk??'ND',x.metrics?.asn??'ND',x.metrics?.gap??'ND',x.metrics?.surplus??'ND','ND — butuh data kelas I–VI/kapasitas/linearitas','Studi kelayakan; belum keputusan merger']));
 }
...
function downloadPdf(m){
 ...
 if(m.study?.visible){
  doc.setFontSize(10);doc.text('Kajian Penataan SD V2 — acuan tunggal',14,y);
  doc.setFontSize(7);doc.text('Acuan: 445 SD Negeri • 58.670 peserta didik • 137 SD ≤90 peserta didik • cut-off 11–13 September 2026.',14,y+4);
  doc.autoTable({startY:y+7,head:[['Klaster','Kec.','PD Acuan','Jarak Kajian','Jarak SIMANTAB','GTK','ABK/ASN berjalan','ABK sesudah']],body:m.study.clusters.map(x=>[x.label,x.district,x.schools.reduce((n,s)=>n+s.students,0),'±'+x.studyDistanceKm+' km',x.liveDistance==null?'Belum ada':x.liveDistance+' km',x.bothVerified?'2/2 terverifikasi':'Belum resmi',x.metrics?(x.metrics.abk+'/'+x.metrics.asn):'ND','ND']),styles:{fontSize:6},headStyles:{fontSize:6}});
