/* SIMANTAB_GTK_REDISTRIBUTION_ANALYSIS_V8 */
(async()=>{
...
const SD_STUDY={
 ...
 legalBasis:'Peraturan Bupati Batang Nomor 1 Tahun 2020 tentang Pendirian, Perubahan, Penggabungan dan Penutupan Program atau Satuan Pendidikan — Pasal 24',
...
function buildStudyModel(data){
 ...
 const totalStudents=sdSchools.reduce((n,s)=>n+num(s.students),0);
 const syncTimes=sdSchools.map(s=>s.synced_at).filter(Boolean).map(x=>new Date(x)).filter(d=>!Number.isNaN(d.getTime()));
 const latestSync=syncTimes.length?new Date(Math.max(...syncTimes.map(d=>d.getTime()))):null;
 return{visible,sdSchools,districtWide,buckets,clusters,totalStudents,latestSync,scopeLabel:districtWide?'Kabupaten':'Cakupan akun'};
}
...
async function fetchData(force=false){
 ...
 const [schoolsQ,needsQ,wfQ]=await Promise.all([
  sb.from('school_master').select('npsn,school_name,school_status,jenjang,bentuk_pendidikan,kecamatan,students,rombel,is_active,synced_at,latitude,longitude,maps_url,location_source,location_verified,location_verified_at').eq('is_active',true).eq('school_status','NEGERI').order('school_name'),
...
function studyClusterRows(study){
 return study.clusters.map((x,i)=>{
  ...
  return'<tr><td>'+(i+1)+'</td><td><b>'+esc(x.label)+'</b><div class="gar-small">'+esc(x.access)+'</div></td><td>'+esc(x.district)+'</td><td>'+current+'</td><td>'+esc(live)+'</td><td>'+gtk+'</td><td>'+abk+'</td><td>'+gap+'</td><td><span class="gar-study-nd">ND</span><div class="gar-small">Butuh murid per kelas I–VI, kapasitas ruang, dan linearitas.</div></td><td><span class="gar-pill orange">Studi kelayakan</span><div class="gar-small">Belum keputusan merger.</div></td></tr>';
 }).join('');
}
function studySection(m){
 ...
 const syncLabel=s.latestSync?s.latestSync.toLocaleDateString('id-ID',{day:'2-digit',month:'long',year:'numeric'}):'belum tercatat';
 return'<div id="garStudyPanel" class="gar-card gar-wide"><div class="gar-study-title"><div><div class="gar-label">INTEGRASI KAJIAN PENATAAN SD</div><h3 style="margin:5px 0 4px;color:#0f3f76">Penataan SD ↔ Redistribusi GTK</h3><div class="gar-small">Metode dan legal gate mengikuti Kajian V2, tetapi <b>seluruh hitungan operasional memakai satu sumber angka: SIMANTAB Online</b>. Angka baseline lama tidak ditampilkan sebagai pembanding agar tidak menimbulkan dua versi hitungan.</div></div><span class="gar-study-flag">DATA SIMANTAB • sinkron '+esc(syncLabel)+'</span></div>'+
 '<div class="gar-study-kpis"><div class="gar-study-kpi"><span>SD Negeri aktif</span><b>'+fmt(currentCount)+'</b></div><div class="gar-study-kpi"><span>Peserta didik</span><b>'+fmt(s.totalStudents)+'</b></div><div class="gar-study-kpi"><span>SD ≤90 peserta didik</span><b>'+fmt(currentLow)+'</b></div><div class="gar-study-kpi"><span>SD ≤120 peserta didik</span><b>'+fmt(s.buckets.le90+s.buckets.b91_120)+'</b></div><div class="gar-study-kpi"><span>Klaster Tahap I</span><b>'+fmt(SD_STUDY.clusters.length)+'</b></div></div>'+
 '<div class="gar-note gar-study-ok"><b>Satu hitungan resmi modul ini: SIMANTAB.</b> Jumlah sekolah, peserta didik, screening, ABK/ASN berjalan, gap, dan surplus diambil dari data SIMANTAB yang aktif pada saat halaman dibuka. Dokumen kajian tetap dipakai untuk metodologi, legal gate, dan daftar klaster studi kelayakan.</div>'+
 ...
 '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px"><div><div class="gar-label">LEGAL GATE PASAL 24 — PERBUP BATANG NOMOR 1 TAHUN 2020</div><div class="gar-small" style="margin:4px 0 7px"><b>Dasar hukum:</b> '+esc(SD_STUDY.legalBasis)+'</div><div class="gar-table-wrap" style="max-height:360px"><table class="gar-table"><thead><tr><th>Kode</th><th>Syarat</th><th>Status Kajian V2</th></tr></thead><tbody>'+studyLegalRows()+'</tbody></table></div></div><div><div class="gar-label">SCREENING KUANTITATIF — SIMANTAB</div><div class="gar-table-wrap" style="max-height:360px"><table class="gar-table"><thead><tr><th>Kategori</th><th>Jumlah SD</th><th>Persentase</th></tr></thead><tbody><tr><td>≤60</td><td><b>'+fmt(s.buckets.le60)+'</b></td><td>'+pct(s.buckets.le60,currentCount)+'%</td></tr><tr><td>61–90</td><td><b>'+fmt(s.buckets.b61_90)+'</b></td><td>'+pct(s.buckets.b61_90,currentCount)+'%</td></tr><tr><td>91–120</td><td><b>'+fmt(s.buckets.b91_120)+'</b></td><td>'+pct(s.buckets.b91_120,currentCount)+'%</td></tr><tr><td>&gt;120</td><td><b>'+fmt(s.buckets.gt120)+'</b></td><td>'+pct(s.buckets.gt120,currentCount)+'%</td></tr></tbody></table></div><div class="gar-small" style="margin-top:7px">Kategori adalah alat screening, bukan syarat hukum penggabungan.</div></div></div>'+
 '<div style="margin-top:12px"><div class="gar-label">EMPAT KLASTER TAHAP I — ANGKA SIMANTAB</div><div class="gar-table-wrap"><table class="gar-table"><thead><tr><th>No</th><th>Klaster</th><th>Kec.</th><th>Peserta Didik</th><th>Jarak SIMANTAB</th><th>Status GTK</th><th>ABK/ASN berjalan</th><th>Gap/Surplus berjalan</th><th>ABK sesudah skenario</th><th>Status</th></tr></thead><tbody>'+studyClusterRows(s)+'</tbody></table></div></div>'+
 '<div class="gar-study-source" style="margin-top:10px"><b>Sumber angka:</b> SIMANTAB Online (school_master dan kebutuhan GTK aktif; sinkron master sekolah '+esc(syncLabel)+'). <b>Sumber metodologi/legal:</b> '+esc(SD_STUDY.title)+' — '+esc(SD_STUDY.version)+', '+esc(SD_STUDY.date)+'; '+esc(SD_STUDY.legalBasis)+'.</div></div>';
}
...
function downloadCsv(m){
 ...
 if(m.study?.visible){
  const syncLabel=m.study.latestSync?m.study.latestSync.toLocaleDateString('id-ID'):'belum tercatat';
  lines.push([],['KAJIAN PENATAAN SD — SATU HITUNGAN SIMANTAB'],['Sumber angka','SIMANTAB Online'],['Sinkron master sekolah',syncLabel],['SD Negeri aktif',m.study.sdSchools.length],['Peserta didik',m.study.totalStudents],['SD ≤60',m.study.buckets.le60],['SD 61–90',m.study.buckets.b61_90],['SD 91–120',m.study.buckets.b91_120],['SD >120',m.study.buckets.gt120],['SD ≤90',m.study.buckets.le90],[],['KLASTER TAHAP I — STUDI KELAYAKAN, ANGKA SIMANTAB'],['Klaster','Kecamatan','Peserta Didik SIMANTAB','Jarak SIMANTAB km','Status GTK','ABK Berjalan','ASN Berjalan','Gap Berjalan','Surplus Berjalan','ABK Sesudah Skenario','Status']);
  m.study.clusters.forEach(x=>lines.push([x.label,x.district,x.bothFound?x.found.reduce((n,s)=>n+num(s.students),0):'Belum lengkap',x.liveDistance??'Belum ada',x.bothVerified?'2/2 terverifikasi':'Belum resmi',x.metrics?.abk??'ND',x.metrics?.asn??'ND',x.metrics?.gap??'ND',x.metrics?.surplus??'ND','ND — butuh data kelas I–VI/kapasitas/linearitas','Studi kelayakan; belum keputusan merger']));
 }
...
function downloadPdf(m){
 ...
 if(m.study?.visible){
  doc.setFontSize(10);doc.text('Kajian Penataan SD — satu hitungan SIMANTAB',14,y);
  doc.setFontSize(7);doc.text('Angka operasional hanya memakai SIMANTAB. Klaster Tahap I tetap objek studi kelayakan; ABK pasca-merger menunggu legal gate dan data kelas I–VI lengkap.',14,y+4);
  doc.autoTable({startY:y+7,head:[['Klaster','Kec.','Peserta Didik SIMANTAB','Jarak SIMANTAB','GTK','ABK/ASN berjalan','ABK sesudah']],body:m.study.clusters.map(x=>[x.label,x.district,x.bothFound?x.found.reduce((n,s)=>n+num(s.students),0):'Belum lengkap',x.liveDistance==null?'Belum ada':x.liveDistance+' km',x.bothVerified?'2/2 terverifikasi':'Belum resmi',x.metrics?(x.metrics.abk+'/'+x.metrics.asn):'ND','ND']),styles:{fontSize:6},headStyles:{fontSize:6}});
