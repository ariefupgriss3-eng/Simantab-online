...
async function load(){const body=$('schoolMasterBody');if(!body)return;body.innerHTML='<div class="card"><div class="small">Memuat Master Sekolah…</div></div>';const {data,error}=await sb.from('school_master').select('*').order('school_name').limit(1500);if(error){body.innerHTML=`<div class="card err">${esc(error.message)}</div>`;return}rows=(data||[]).filter(r=>r.is_active!==false);render()}
...
window.__simantabSchoolMasterRestoreFix={version:2,schemaAgnostic:true};
})();