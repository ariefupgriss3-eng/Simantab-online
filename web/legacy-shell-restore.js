...
function ensureSections(){
 ...
 ensureSection('adminData','Pusat Kelola Data Admin','Ringkasan dan navigasi data operasional SIMANTAB. Perubahan data dilakukan melalui modul sumber sesuai kewenangan.','adminDataBody');
}
...
async function loadActivities(){
 ...
 const {data,error}=await sb.from('field_activities').select('id,activity_name,activity_date,activity_time,activity_end_date,activity_end_time,place,document_number,speakers,updated_at').order('activity_date',{ascending:false}).limit(100);
 if(error){body.innerHTML=`<div class="card err">${esc(error.message)}</div>`;return}
 const rows=data||[];
 const form=`<div class="card" style="margin-bottom:12px"><h3 style="margin-top:0">Tambah Kegiatan Bidang</h3><div class="info" style="margin-bottom:12px"><b>Mode operasional aktif.</b> Simpan dan ubah data mengikuti kewenangan akun serta kebijakan RLS SIMANTAB.</div><div class="grid"><div class="field s8"><label>1. Nama Kegiatan</label><input id="actName" maxlength="250" placeholder="Nama kegiatan"></div><div class="field s4"><label>Nomor Dokumen/Undangan</label><input id="actNumber" placeholder="Nomor surat/dokumen"></div><div class="field s4"><label>2. Tanggal</label><input id="actDate" type="date"></div><div class="field s4"><label>Jam Mulai</label><input id="actTime" type="time"></div><div class="field s4"><label>3. Tempat</label><input id="actPlace" placeholder="Tempat kegiatan"></div><div class="field s6"><label>4A. Peserta — Nama Personal</label><textarea id="actParticipantsPersonal" placeholder="Satu nama per baris"></textarea></div><div class="field s6"><label>4B. Peserta — Jabatan</label><textarea id="actParticipantPositions" placeholder="Satu jabatan/kelompok per baris"></textarea></div><div class="field s6"><label>5. Pejabat yang Diundang</label><textarea id="actOfficials" placeholder="Satu per baris"></textarea></div><div class="field s6"><label>6. Narasumber</label><textarea id="actSpeakers" placeholder="Satu nama per baris"></textarea></div><div class="field s12"><label>Deskripsi Kegiatan</label><textarea id="actDescription"></textarea></div><div class="field s12"><label>Materi</label><textarea id="actMaterials"></textarea></div><div class="field s12"><label>Laporan Ringkas</label><textarea id="actReport"></textarea></div><div class="field s6"><label>Latar Belakang</label><textarea id="reportBackground"></textarea></div><div class="field s6"><label>Tujuan</label><textarea id="reportObjectives"></textarea></div><div class="field s6"><label>Hasil</label><textarea id="reportResults"></textarea></div><div class="field s6"><label>Tindak Lanjut</label><textarea id="reportFollowUp"></textarea></div><div class="field s6"><label>Rekomendasi</label><textarea id="reportRecommendations"></textarea></div><div class="field s6"><label>Penutup</label><textarea id="reportClosing"></textarea></div><div class="field s6"><label>Jabatan Penandatangan</label><input id="actSignTitle" value="${esc(profile().position||'')}"></div><div class="field s6"><label>Nama Penandatangan</label><input id="actSignName" value="${esc(profile().full_name||'')}"></div><div class="field s6"><label>NIP Penandatangan</label><input id="actSignNip" value="${esc(profile().nip||'')}"></div><div class="field s6"><label>Unit Penandatangan</label><input id="actSignUnit" value="${esc(profile().unit||'Dinas Pendidikan dan Kebudayaan')}"></div></div><div id="actMsg" class="small" style="margin:8px 0"></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" onclick="saveAuthorizedActivityInput()">Simpan Terintegrasi</button><button class="btn soft" onclick="cancelActivityEdit()">Batal/Segarkan</button></div></div>`;
 const list=`<div class="card"><h3 style="margin-top:0">Daftar Kegiatan</h3>${rows.length?`<div class="tablewrap"><table><thead><tr><th>Tanggal</th><th>Kegiatan</th><th>Tempat</th><th>Nomor</th><th>Narasumber</th><th>Update</th><th>Aksi</th></tr></thead><tbody>${rows.map(a=>`<tr><td>${esc(a.activity_date||'-')}<br><span class="small">${esc(a.activity_time||'')}</span></td><td><b>${esc(a.activity_name)}</b></td><td>${esc(a.place||'-')}</td><td>${esc(a.document_number||'-')}</td><td>${esc((a.speakers||[]).join(', ')||'-')}</td><td>${fmt(a.updated_at)}</td><td><div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn soft" onclick="editActivity('${a.id}')">Ubah</button><button class="btn soft" onclick="deleteActivity('${a.id}')">Hapus</button></div></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">Belum ada kegiatan.</div>'}</div>`;
 body.innerHTML=form+list;
}
window.cancelActivityEdit=async()=>window.showTab('activities');
window.deleteActivity=async id=>{if(profile().role!=='KABID'){alert('Penghapusan Kegiatan Bidang hanya dapat dilakukan oleh Kabid.');return}if(!confirm('Hapus kegiatan ini? Tindakan ini tidak dapat dibatalkan.'))return;const {error}=await sb.from('field_activities').delete().eq('id',id);if(error){alert(error.message);return}await window.showTab('activities');};
...
async function loadAdminData(){
 ...
 body.innerHTML=`<div class="card" style="margin-bottom:12px"><div class="info"><b>Pusat Kelola Data aktif.</b> Halaman ini berfungsi sebagai ringkasan kontrol. Revisi, persetujuan, dan penghapusan dilakukan dari modul sumber agar alur audit dan RLS tetap terjaga.</div></div><div class="grid">${cards.map(x=>`<div class="card s4"><div class="label">${esc(x[0])}</div><div class="metric">${esc(x[1])}</div>${x[2]?`<div class="small">${esc(x[2])}</div>`:''}</div>`).join('')}</div>`;
}
...
window.__simantabLegacyShell={version:2,restored:['activities','attendance','schoolMaster','team','adminData'],previewReadOnly:false,activitiesOperational:true,adminDataControlCenter:true};
})();