...
const dashboardUiRefreshFile='dashboard-ui-refresh.js';
const dashboardUiRefreshCode=await fs.readFile(new URL(`./${dashboardUiRefreshFile}`,import.meta.url),'utf8');
if(!/SIMANTAB_DASHBOARD_UI_REFRESH_V1/.test(dashboardUiRefreshCode))throw new Error('Dashboard UI refresh v1 tidak valid.');
await fs.writeFile(path.join(staticDir,dashboardUiRefreshFile),dashboardUiRefreshCode);
