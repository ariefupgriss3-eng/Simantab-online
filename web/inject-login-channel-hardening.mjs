...
if(!code.includes('SIMANTAB_LOGIN_CHANNEL_HARDENING_V2'))throw new Error('Modul login channel hardening v2 tidak valid.');
...
html=html.slice(0,bodyClose)+`<script type="module" src="./login-channel-hardening.js?v=2"></script>\n`+html.slice(bodyClose);
await fs.writeFile(`.vercel/output/static/${moduleName}`,code);
await fs.writeFile(outputPath,html);
console.log(JSON.stringify({loginChannelHardening:true,version:2,gtkLogin:['username','email'],strictChannelValidation:true}));
