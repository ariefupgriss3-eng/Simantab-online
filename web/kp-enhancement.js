...
(async()=>{
  ...
  async function renderKpForm(){
    ...
    try{
      ...
      section.innerHTML=`<div class="head"><div><h2>Usul Kenaikan Pangkat</h2><p>Form khusus sesuai SOP KP. Setiap dokumen maksimal 1 MB.</p></div><button class="btn soft" onclick="showTab('services')">← Kembali</button></div>
      ...
      <div class="card" style="margin-bottom:13px"><h3 style="margin-top:0">A. Data Usulan</h3><div class="grid">
        ...
        <div class="field s4"><label>Jenis Usul</label><select id="kpProposalType"><option value="REGULER">KP Reguler</option><option value="BERSAMAAN_KENAIKAN_JENJANG">Bersamaan Kenaikan Jenjang</option></select></div>
        <div class="field s4"><label>TMT KP Terakhir</label><input id="kpLastPromotionDate" type="date"></div>
      ...
      <div class="card" style="margin-bottom:13px"><h3 style="margin-top:0">C. Unggah Dokumen KP</h3><div class="small" style="margin-bottom:8px">PDF/JPG/PNG maksimal 1 MB per dokumen. Dokumen bertanda * wajib.</div>
  ...
  window.submitKpSop=async()=>{
    ...
    try{
      ...
      for(const input of inputs){
        ...
        if(f && (f.size>1048576||!['application/pdf','image/jpeg','image/png'].includes(f.type))) throw new Error(`${f.name}: ukuran harus ≤ 1 MB dan format PDF/JPG/PNG.`);
      }
      ...
      const detail={proposal_type:proposalType,employee_nip:document.getElementById('kpNip').value.trim(),last_rank:document.getElementById('kpLastRank').value.trim(),last_position:document.getElementById('kpLastPosition').value.trim(),last_promotion_date:document.getElementById('kpLastPromotionDate').value||null,proposed_rank:document.getElementById('kpProposedRank').value.trim(),proposed_position:document.getElementById('kpProposedPosition').value.trim(),competency_test_required:compReq,competency_test_passed:compReq?document.getElementById('kpCompetencyPassed').checked:null,eligibility_declared:true,ak_siasn_declared:true,skp_siasn_declared:true,notes:document.getElementById('kpNotes').value.trim(),updated_by:profile.id,updated_at:new Date().toISOString()};
  ...
  window.__openKpProcess=async id=>{
    ...
    try{
      ...
      <div class="card" style="margin-bottom:12px">${kpSteps(d.workflow_stage)}<div class="grid"><div class="s4"><b>Jenis:</b> ${esc(d.proposal_type)}</div><div class="s4"><b>Pangkat terakhir:</b> ${esc(d.last_rank||'-')}</div><div class="s4"><b>Usulan:</b> ${esc(d.proposed_rank||'-')}</div></div></div>
      <div class="card" style="margin-bottom:12px"><h3 style="margin-top:0">Checklist Persyaratan</h3><div class="tablewrap"><table><thead><tr><th>No</th><th>Persyaratan</th><th>Berkas/Konfirmasi</th><th>Status</th><th>Catatan</th>${isDinas?'<th>Verifikasi</th>':''}</tr></thead><tbody>${b.checks.map((x,i)=>{const r=x.service_requirements||{},f=x.file_id?fileById[x.file_id]:null;let proof=f?`<button class="btn soft" onclick="__openKpFile('${encodeURIComponent(f.storage_path)}')">📎 ${esc(f.file_name)}</button>`:(r.code==='ELIGIBILITY_2Y_OR_JENJANG'?(d.eligibility_declared?'✅ Dikonfirmasi GTK':'❌ Belum'):(r.code==='SINKRON_AK_SIASN'?(d.ak_siasn_declared?'✅ Dikonfirmasi GTK':'❌ Belum'):(r.code==='KIRIM_SKP_SIASN'?(d.skp_siasn_declared?'✅ Dikonfirmasi GTK':'❌ Belum'):'Belum ada berkas')));return `<tr><td>${r.sort_order||i+1}</td><td>${esc(r.label||'')}</td><td>${proof}</td><td>${statusBadge(x.status)}</td><td>${esc(x.note||'-')}</td>${isDinas?`<td><select id="kpcheck-${x.id}" style="padding:6px;border:1px solid #dfe7ef;border-radius:8px"><option value="PENDING" ${x.status==='PENDING'?'selected':''}>Pending</option><option value="MS" ${x.status==='MS'?'selected':''}>MS</option><option value="TMS" ${x.status==='TMS'?'selected':''}>TMS</option></select><input id="kpnote-${x.id}" value="${esc(x.note||'')}" placeholder="Catatan" style="margin-top:5px;padding:6px;border:1px solid #dfe7ef;border-radius:8px;max-width:180px"><button class="btn soft" style="margin-top:5px" onclick="__saveKpCheck(${x.id})">Simpan</button></td>`:''}</tr>`}).join('')}</tbody></table></div></div>
