/* SIMANTAB_OFFLINE_CONSULTATION_V1 */
(async()=>{
...
function ensureSection(){
 ...
 sec.innerHTML='<div class="head"><div><h2>Daftar Konsultasi Luring</h2><p>Pendaftaran konsultasi tatap muka Bidang Ketenagaan untuk layanan Diklat KS/BCKS.</p></div></div><div id="offlineConsultationBody"></div>';
...
function ensureNav(){
 ...
 const dik=nav.querySelector('[data-tab="diklatKsBcks"]');dik?dik.after(b):nav.appendChild(b);
}
async function prefill(){
 ...
 return {detail:a[0].data||{},ticket:a[1].data||null};
}
...
function formHtml(d){
 return '<div class="card"><div class="info" style="margin-bottom:12px"><b>Konsultasi luring hanya untuk hal yang memerlukan tatap muka.</b><br>Seluruh proses administrasi tetap dilakukan melalui SIMANTAB dan RGTK.</div><div class="grid">'+
...
async function render(){
 ...
 try{const x=await prefill();body.innerHTML=x.ticket?ticketHtml(x.ticket):formHtml(x.detail);if(!x.ticket)$('ocTakeQueue')?.addEventListener('click',takeQueue)}
 catch(e){body.innerHTML='<div class="card err">'+esc(e.message||e)+'</div>'}
}
async function takeQueue(){
 const btn=$('ocTakeQueue'),msg=$('ocMsg');
 const vals={p_full_name:$('ocFullName')?.value.trim(),p_nip:$('ocNip')?.value.trim(),p_pangkat_golruang:$('ocPangkat')?.value.trim(),p_unit_kerja:$('ocUnit')?.value.trim(),p_topic:$('ocTopic')?.value.trim()};
 if(!vals.p_full_name||!vals.p_nip||!vals.p_pangkat_golruang||!vals.p_unit_kerja||!vals.p_topic){msg.className='err';msg.textContent='Lengkapi seluruh isian sebelum mengambil nomor antrian.';return}
 btn.disabled=true;msg.className='small';msg.textContent='Menentukan nomor antrian dan jadwal…';
 try{const r=await sb.rpc('take_offline_consultation_queue',vals);if(r.error)throw r.error;const t=Array.isArray(r.data)?r.data[0]:r.data;if(!t)throw new Error('Nomor antrian belum berhasil dibuat.');await render()}
 catch(e){msg.className='err';msg.textContent=e.message||String(e)}
 finally{btn.disabled=false}
}
function injectDiklatShortcut(){
 const body=$('diklatKsBcksBody');if(!body||$('offlineConsultationShortcut'))return;
 const c=document.createElement('div');c.id='offlineConsultationShortcut';c.className='card';c.style.marginTop='12px';
 c.innerHTML='<div class="head" style="margin-bottom:0"><div><div class="label">KONSULTASI LURING</div><h3 style="margin:4px 0">🎟️ Daftar Konsultasi Luring</h3><div class="small">Jika konsultasi tatap muka diperlukan, daftar untuk memperoleh nomor antrian, hari, tanggal, jam, dan nama petugas yang melayani.</div></div><button class="btn primary" onclick="showTab(\'offlineConsultation\')">Daftar / Lihat Antrian</button></div>';
 body.appendChild(c);
}
const priorShow=window.showTab;
window.showTab=async function(id){ensureSection();ensureNav();const r=await priorShow.apply(this,arguments);await wait(50);if(id==='offlineConsultation')await render();if(id==='diklatKsBcks')injectDiklatShortcut();return r};
ensureStyle();ensureSection();ensureNav();
for(const ms of [120,400,1000])setTimeout(()=>{ensureSection();ensureNav();injectDiklatShortcut()},ms);
if(document.querySelector('#offlineConsultation.active'))await render();
window.__simantabOfflineConsultation={version:1,applicantOnly:true};
})();